...
<%
  ...
  PreparedStatement stmt = conn.prepareStatement("SELECT * FROM music ORDER BY id DESC");
...
<head>
  ...
  <title>Bangladeshi Music & Artists - Explore Bangladesh</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" />
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" />
  <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&family=Roboto:wght@300;400&display=swap" rel="stylesheet" />
  <style>
    body {
      background: linear-gradient(180deg, #3b1e54 0%, #5e2a84 100%);
    ...
    .btn-comment {
      border-radius: 50px;
      padding: 10px 20px;
      background: linear-gradient(45deg, #8e44ad, #6b2a9d);
      border: none;
      font-size: 0.9rem;
      font-weight: 400;
      color: #fff;
    ...
    .btn-comment:hover {
      background: linear-gradient(45deg, #6b2a9d, #4b1e7d);
      transform: scale(1.05);
    }
    .comment-section {
      max-height: 280px;
...
<div class="intro-section">
  <h2 class="header-title">
    <i class="fas fa-music me-2"></i>Bangladeshi Music & Artists
  </h2>
  <p class="intro-text">Immerse yourself in the soulful melodies and rhythms of Bangladesh</p>
</div>
...
<%
  while (rs.next()) {
    ...
    String musicDesc = rs.getString("description");
    String musicImg = rs.getString("image");

    // Likes count
    PreparedStatement likeCountStmt = conn.prepareStatement(
      "SELECT COUNT(*) AS total FROM likes WHERE item_type = 'music' AND item_id = ?");
    likeCountStmt.setInt(1, musicId);
    ...
    if (currentUserId != null) {
      PreparedStatement userLikeStmt = conn.prepareStatement(
        "SELECT * FROM likes WHERE user_id = ? AND item_id = ? AND item_type = 'music'");
      userLikeStmt.setInt(1, currentUserId);
      userLikeStmt.setInt(2, musicId);
  ...
  <div class="row g-0">
    <div class="col-md-4">
      <div class="card-img-wrapper">
        <img src="<%= musicImg %>" class="card-img" alt="<%= musicName %>" onerror="this.src='images/default.jpg'">
        <div class="card-img-overlay"></div>
      </div>
    </div>
    <div class="col-md-8">
      <div class="card-body">
        <h5 class="card-title"><i class="fas fa-guitar me-2"></i><%= musicName %></h5>
        <p class="card-text"><%= musicDesc %></p>

        <!-- Like Button -->
        <form action="like_toggle.jsp" method="post" style="display:inline;">
          <input type="hidden" name="item_id" value="<%= musicId %>"/>
        ...
        <form action="comment_add.jsp" method="post" class="mt-4">
          ...
          <div class="input-group">
            <textarea name="content" class="form-control comment-box" rows="3" placeholder="Share your thoughts on this music..." required></textarea>
        ...
        <div class="comment-section">
          <%
            PreparedStatement cStmt = conn.prepareStatement(
              "SELECT c.*, u.name FROM comment c JOIN user u ON c.user_id = u.id WHERE c.item_type = 'music' AND c.item_id = ? ORDER BY c.timestamp DESC");
