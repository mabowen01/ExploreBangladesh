...
<head>
  ...
  <title>Popular Sports - Explore Bangladesh</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" />
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" />
  <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&family=Roboto:wght@300;400&display=swap" rel="stylesheet" />
  <style>
    body {
      background: linear-gradient(180deg, #2a6f2a 0%, #4a9e4a 100%);
...
<div class="intro-section">
  <h2 class="header-title">
    <i class="fas fa-trophy me-2"></i>Popular Sports of Bangladesh
  </h2>
  <p class="intro-text">Experience the thrill and passion of Bangladesh's sporting culture</p>
</div>

<%
  while (rs.next()) {
    int sportId = rs.getInt("id");
    String sportName = rs.getString("name");
    String sportDesc = rs.getString("description");
    String sportImg = rs.getString("image");

    // Likes count
    PreparedStatement likeCountStmt = conn.prepareStatement(
      "SELECT COUNT(*) AS total FROM likes WHERE item_type = 'sport' AND item_id = ?");
    likeCountStmt.setInt(1, sportId);
    ...
    if (currentUserId != null) {
      PreparedStatement userLikeStmt = conn.prepareStatement(
        "SELECT * FROM likes WHERE user_id = ? AND item_id = ? AND item_type = 'sport'");
      userLikeStmt.setInt(1, currentUserId);
      userLikeStmt.setInt(2, sportId);
  ...
  <div class="row g-0">
    <div class="col-md-4">
      <div class="card-img-wrapper">
        <img src="<%= sportImg %>" class="card-img" alt="<%= sportName %>" onerror="this.src='images/default.jpg'">
        <div class="card-img-overlay"></div>
      </div>
    </div>
    <div class="col-md-8">
      <div class="card-body">
        <h5 class="card-title"><i class="fas fa-running me-2"></i><%= sportName %></h5>
        <p class="card-text"><%= sportDesc %></p>

        <!-- Like Button -->
        <form action="like_toggle.jsp" method="post" style="display:inline;">
          <input type="hidden" name="item_id" value="<%= sportId %>"/>
        ...
        <form action="comment_add.jsp" method="post" class="mt-4">
          ...
          <div class="input-group">
            <textarea name="content" class="form-control comment-box" rows="3" placeholder="Share your sports experience..." required></textarea>
        ...
        <div class="comment-section">
          <%
            PreparedStatement cStmt = conn.prepareStatement(
              "SELECT c.*, u.name FROM comment c JOIN user u ON c.user_id = u.id WHERE c.item_type = 'sport' AND c.item_id = ? ORDER BY c.timestamp DESC");
            cStmt.setInt(1, sportId);
